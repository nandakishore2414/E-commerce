...
function ProductCard({ product }) {
  ...
  const handleAddToCart = async () => {
    try {
      setError("");
      setAdding(true);
      await.post("/user/cart", { productId: product._id, quantity: 1 });
      navigate("/cart");
    } catch (err) {
  ...
  return (
    <div className="group relative border border-gray-100 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="aspect-h-4 aspect-w-3 w-full overflow-hidden rounded-md bg-gray-100 lg:aspect-h-8 lg:aspect-w-7">
        <img
          src={product.image ? `/uploads/${product.image}` : (product.imageUrl || 'https://via.placeholder.com/600x800.png?text=Product+Image')}
          alt={product.name}
...
function ProductList() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        const res = await.get('/user/products');
